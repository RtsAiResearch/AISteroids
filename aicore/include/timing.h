 ...
 * @file
 *
 * Holds the timing system for the AI library. This implementations of
 * the components in this file are very likely to need replacing for
 * different platforms, since the implementation relies on operating
 * system-provided timing services.
 */
#ifndef AICORE_TIMING_H
#define AICORE_TIMING_H

namespace aicore
{
    /**
     * Represents all the information that the demo might need about
     * the timing of the game: current time, fps, frame number, and
     * so on.
     */
    struct TimingData
    {
